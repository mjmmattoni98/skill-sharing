...
import com.losacabaos.skillsharing.email.Email;
import com.losacabaos.skillsharing.email.EmailDao;
...
import com.losacabaos.skillsharing.offer.Offer;
...
import com.losacabaos.skillsharing.student.Student;
import com.losacabaos.skillsharing.student.StudentDao;
...
public class SkillController extends RoleController {
    ...
    public String processAddSkill(@ModelAttribute("skill") Skill skill, Model model,
        ...
        try {
            ...
            bindingResult.rejectValue("name", "duplicated", "This skill with the current level " +
                    "already exists");
    ...
    @GetMapping(value = "/update/{name}/{level}")
    public String updateSkill(HttpSession session, Model model, @PathVariable String name, @PathVariable String level) {
        InternalUser user = checkSession(session, SKP_ROLE);
        if (user == null) {
            model.addAttribute("user", new InternalUser());
            return "login";
        ...
        Skill skill = skillDao.getSkill(name, level);
        model.addAttribute("skill", skill);
        model.addAttribute("skillLevels", loadSkills());
        return "skill/update";
    }
    ...
    @RequestMapping(value = "/activateDisable/{name}/{level}")
    public String activateOrDisableSkill(HttpSession session, Model model, @PathVariable String name, @PathVariable String level) {
        InternalUser user = checkSession(session, SKP_ROLE);
        if (user == null) {
            model.addAttribute("user", new InternalUser());
            return "login";
