...
public class SkillDao {
    ...
    public void deleteSkill(Skill skill) {
        jdbcTemplate.update("DELETE FROM skill WHERE name = ? AND level = ?::skill_level",
                skill.getName(),
                skill.getLevel()
        );
    }

    public void deleteSkill(String name, String level) {
        jdbcTemplate.update("DELETE FROM skill WHERE name = ? AND level = ?::skill_level",
                name,
                level
        );
    ...
    public void updateSkill(Skill skill) {
        jdbcTemplate.update("UPDATE skill SET description = ?, level = ?::skill_level, canceled = ? " +
                        "WHERE name = ? AND level = ?::skill_level",
                skill.getDescription(),
                skill.getLevel(),
                skill.isCanceled(),
                skill.getName(),
    ...
    public Skill getSkill(String name, String level) {
        try {
            return jdbcTemplate.queryForObject("SELECT * FROM skill WHERE name = ? AND level = ?::skill_level",
                    new SkillRowMapper(),
                    name,
                    level
            );
